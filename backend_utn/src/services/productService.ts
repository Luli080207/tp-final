import { ProductModel, IProduct } from "src/model/ProductModel.ts";
...
export const listProducts = async (filters: Filters) => {
  ...
  const q = ProductModel.find(query).sort({createdAt: -1});
...
export const createProduct = async (payload: Partial<IProduct>) => {
  const p = new ProductModel(payload);
  return await p.save();
};
...
export const updateProduct = async (id: string, payload: Partial<IProduct>) => {return await ProductModel.findByIdAndUpdate(id, payload, {new: true}).exec();
};
